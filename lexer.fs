  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               if lookahead = '.' then
                ...
                let nextToken = nextChar input
                if List.contains nextToken ['0'..'9'] then // next is a number
                  let (lit, next) = collectIntLiteral (nextChar input) input (string nextToken)
                  lexer next input (("real_literal:"+ literal + "." + lit) :: tokens)
                else
                  lexer nextToken input (("real_literal:"+ literal + ".") :: tokens)
